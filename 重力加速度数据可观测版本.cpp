...
void showInfo(const char *str,SDL_Color a,int x,int y,int n){
		...
		SDL_Rect r={x,y-350+load->h*n/2,load->w/1.5,load->h/1.2};
...
void GameInfo(vector<GameObject*>&v)
{
	...
	for(int i=0;i<v.size();++i){
		SetInfo(*v[i],{0,0,0},"obj:",i+1,0);
		SetInfo(*v[i],{0,0,255},"x:",v[i]->x,1);
		SetInfo(*v[i],{0,0,255},"y:",v[i]->y,2);
		SetInfo(*v[i],{0,0,255},"vx:",v[i]->vx,3);
		SetInfo(*v[i],{0,0,255},"vy:",v[i]->vy,4);
		SetInfo(*v[i],{0,0,255},"degrees:",fmod(360+v[i]->degrees,360),5);
		SetInfo(*v[i],{19,12,25},(v[i]->dstObj1!=-1?"collision<-:obj":"NOT Found obj "),v[i]->dstObj1,6);
		SetInfo(*v[i],{19,12,25},(v[i]->dstObj2!=-1?"collision->:obj":"NOT Found obj "),v[i]->dstObj2,7);
		SetInfo(*v[i],{12,22,255},"secondsPassed:", secondsPassed,8);
					SetInfo(*v[i],{12,22,255},"speed:",v[i]->speed,9);
				SetInfo(*v[i],{12,22,255},"impulse:",v[i]->impulse,10);
			SetInfo(*v[i],{12,22,255},"vCollisionNorm.x:",v[i]->vCollisionNorm.x,11);
				SetInfo(*v[i],{12,22,255},"vCollisionNorm.y:",v[i]->vCollisionNorm.y,12);
	}
}
